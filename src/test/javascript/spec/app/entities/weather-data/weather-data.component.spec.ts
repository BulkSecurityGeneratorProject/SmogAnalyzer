/* tslint:disable max-line-length */
import { ComponentFixture, TestBed } from '@angular/core/testing';
import { Observable, of } from 'rxjs';
import { HttpHeaders, HttpResponse } from '@angular/common/http';

import { SmogAnalyzerTestModule } from '../../../test.module';
import { WeatherDataComponent } from 'app/entities/weather-data/weather-data.component';
import { WeatherDataService } from 'app/entities/weather-data/weather-data.service';
import { WeatherData } from 'app/shared/model/weather-data.model';

describe('Component Tests', () => {
    describe('WeatherData Management Component', () => {
        let comp: WeatherDataComponent;
        let fixture: ComponentFixture<WeatherDataComponent>;
        let service: WeatherDataService;

        beforeEach(() => {
            TestBed.configureTestingModule({
                imports: [SmogAnalyzerTestModule],
                declarations: [WeatherDataComponent],
                providers: []
            })
                .overrideTemplate(WeatherDataComponent, '')
                .compileComponents();

            fixture = TestBed.createComponent(WeatherDataComponent);
            comp = fixture.componentInstance;
            service = fixture.debugElement.injector.get(WeatherDataService);
        });

        it('Should call load all on init', () => {
            // GIVEN
            const headers = new HttpHeaders().append('link', 'link;link');
            spyOn(service, 'query').and.returnValue(
                of(
                    new HttpResponse({
                        body: [new WeatherData(123)],
                        headers
                    })
                )
            );

            // WHEN
            comp.ngOnInit();

            // THEN
            expect(service.query).toHaveBeenCalled();
            expect(comp.weatherData[0]).toEqual(jasmine.objectContaining({ id: 123 }));
        });
    });
});
